...
export default {
  init() {
    ...
    events.on(consts.events.WIPE_TESTING_CODE, payload => {
      if(payload.position) {
        ...
        if(amount == 0) {
          chat.debug('No testing blocks to clear.');
          return;
        }
  ...
  async testCode(position, code, item) {
    ...
    try {
      ...
      chat.debug('Error executing code', err);
  ...
  async _verifyPosition(position, id) {
    ...
    if(id != expectedId) {
      ...
      chat.debug(`Block at position ${position.join('|')} with local code should be of type ${expectedId} but it's ${id}. We're deleting the local code:\n ${localCode}`);
      this.clearTestingCode(position);
    } else {
